...
class DataLoader:
    ...
    def CreateDataset(self):
        ...
        train_val_records = records[:int(len(records)*0.1)*self.opt.fold] + records[int(len(records)*0.1)*(self.opt.fold+1):]
        train_records = train_val_records[int(len(train_val_records)*0.1):]
        val_records = train_val_records[:int(len(train_val_records)*0.1)]
        test_records = records[int(len(records)*0.1)*self.opt.fold: int(len(records)*0.1)*(self.opt.fold+1)]
